...
let singleton_modules ~ocamldep_modules_data m =
...
let modules t ~for_ =
  let { modules; dep_graphs = _; ocamldep_modules_data = _ } =
...
let dep_graphs t ~for_ =
  let { dep_graphs; modules = _; ocamldep_modules_data = _ } =
    Lib_mode.By_mode.get t.modules ~for_ |> Option.value_exn
  in
  dep_graphs
;;

let ocamldep_modules_data t ~for_ =
  let { ocamldep_modules_data; modules = _; dep_graphs = _ } =
    Lib_mode.By_mode.get t.modules ~for_ |> Option.value_exn
  in
  ocamldep_modules_data
;;
...
let create
  ...
  let+ ocaml_modules =
    ...
    | Some modules ->
      let ocamldep_modules_data : Ocamldep.Modules_data.t =
        ...
        ; obj_dir
        ; sctx = super_context
        ; vimpl = Option.bind vimpl ~f:(fun (vimpl : _ Lib_mode.By_mode.t) -> vimpl.ocaml)
        ; modules
        ; stdlib
        }
      in
  ...
  and+ melange_modules =
    ...
    | Some modules ->
      let ocamldep_melange_modules_data : Ocamldep.Modules_data.t =
        { Ocamldep.Modules_data.dir = Obj_dir.dir obj_dir
        ; sandbox
        ; obj_dir
        ; sctx = super_context
        ; vimpl =
...
let for_alias_module t alias_module ~for_ =
  ...
  let (modules, includes) : modules * Includes.t =
    ...
    | false ->
      let ocamldep_modules_data = ocamldep_modules_data ~for_ t in
      singleton_modules ~ocamldep_modules_data alias_module, Includes.empty
    | true ->
...
let for_root_module t root_module ~for_ =
  ...
  ; modules =
      (let new_modules =
         singleton_modules
           root_module
           ~ocamldep_modules_data:(ocamldep_modules_data t ~for_)
       in
...
let for_module_generated_at_link_time t ~requires ~module_ ~for_ =
  ...
  let modules =
    let modules =
      let ocamldep_modules_data = ocamldep_modules_data t ~for_ in
      Some (singleton_modules module_ ~ocamldep_modules_data)
    in
