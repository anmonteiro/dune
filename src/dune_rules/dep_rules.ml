...
let transitive_deps_output_from_immediate_deps t immediate_deps =
  let transitive =
...
and transitive_deps_output_uncached t unit ~ml_kind =
  ...
  let* immediate_deps =
    Ocamldep.read_immediate_deps_of
      ...
      unit
  in
  transitive_deps_output_from_immediate_deps t immediate_deps
    ...
    dict_of_func_concurrently (fun ~ml_kind ->
      let deps =
        ...
        then Action_builder.return []
          ...
          in
          let+ output =
            transitive_deps_output_from_immediate_deps transitive_deps immediate_deps
          in
