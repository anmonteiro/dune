...
type include_state =
  | Source of
      { context : Path.Source.t Include_stanza.context
      ; current_file : Path.Source.t
      }
  | Build of
      { context : Path.Build.t Include_stanza.context
      ; current_file : Path.Build.t
      }

let include_state_of_stanza_current_file = function
  | Stanzas.Enabled_if_stanza.Include_context.Source current_file ->
    Source { context = Include_stanza.in_src_file current_file; current_file }
  | Stanzas.Enabled_if_stanza.Include_context.Build current_file ->
    Build { context = Include_stanza.in_build_file current_file; current_file }
;;

let enabled_if_include_context_of_include_state = function
  | Source { current_file; _ } ->
    Stanzas.Enabled_if_stanza.Include_context.Source current_file
  | Build { current_file; _ } ->
    Stanzas.Enabled_if_stanza.Include_context.Build current_file
;;

let parser_context_of_include_state include_state =
  Univ_map.singleton
    Stanzas.Enabled_if_stanza.Include_context.key
    (enabled_if_include_context_of_include_state include_state)
;;

(* XXX this is needed for evaluating includes generated by dune files written
   in OCaml syntax.*)
let rec parse_file_includes ~stanza_parser include_state sexps =
  let parser_context = parser_context_of_include_state include_state in
  ...
  |> Memo.List.concat_map ~f:(fun stanza ->
    ...
    | Stanzas.Include.T (loc, fn) ->
      (match include_state with
       | Source { context; _ } ->
         let current_file = Include_stanza.file_path context loc fn in
         let* sexps, context = Include_stanza.load_sexps ~context (loc, fn) in
         parse_file_includes ~stanza_parser (Source { context; current_file }) sexps
       | Build { context; _ } ->
         let current_file = Include_stanza.file_path context loc fn in
         let* sexps, context = Include_stanza.load_sexps ~context (loc, fn) in
         parse_file_includes ~stanza_parser (Build { context; current_file }) sexps)
    | _ -> Memo.return [ stanza ])
;;

let parse_stanzas ~include_state ~(eval : eval) sexps =
  ...
  let* stanzas =
    ...
    in
    parse_file_includes ~stanza_parser include_state sexps
  in
...
let parse sexps ~file ~(eval : eval) ~context_name =
  let include_state =
    let file =
      match file with
      | Some f -> f
    ...
    in
    Source { context = Include_stanza.in_src_file file; current_file = file }
  in
  let+ stanzas, dynamic_includes = parse_stanzas sexps ~include_state ~eval in
  ( { dir = eval.dir
    ; context_name
...
module Make_fold (M : Monad.S) = struct
  ...
  let rec fold_static_stanzas l ~init ~f =
    match l with
    | [] -> M.return init
    | t :: l -> inner_fold t (static_stanzas t) l ~init ~f
...
let rec eval_enabled_if_stanzas stanzas ~stanza_parser ~eval_enabled_if =
  Memo.List.concat_map stanzas ~f:(fun stanza ->
    match Stanza.repr stanza with
    | Stanzas.Enabled_if_stanza.T { enabled_if; stanzas; current_file; _ } ->
      let* enabled = eval_enabled_if enabled_if in
      if not enabled
      then Memo.return []
      else
        let* stanzas =
          parse_file_includes
            ~stanza_parser
            (include_state_of_stanza_current_file current_file)
            stanzas
        in
        eval_enabled_if_stanzas stanzas ~stanza_parser ~eval_enabled_if
    | _ -> Memo.return [ stanza ])
;;

let stanzas t =
  let* stanzas = Memo.Lazy.force t.stanzas in
  let dir = Path.Build.append_source (Context_name.build_dir t.context_name) t.dir in
  let* expander = Expander0.get ~dir in
  let stanza_parser = Dune_project.stanza_parser ~dir:t.dir t.project in
  eval_enabled_if_stanzas stanzas ~stanza_parser ~eval_enabled_if:(fun enabled_if ->
...
module Script = struct
  ...
  let eval_one ~context_name ~context { file; from_parent; eval } =
    ...
    |> parse ~file:(Some file) ~eval ~context_name
  ;;
end
...
module Eval = struct
  type script =
    | Plain of
        ...
        ; static : Dune_lang.Ast.t list
        }
    | Script of Script.t

  open Memo.O

  let classify ~eval dune_file =
    ...
    | Plain -> Memo.return (Plain { eval; file; static })
    | Ocaml_script ->
      Memo.return
        (Script
           { eval
  ...
  let rec collect_dynamic_includes (eval : eval) include_context dynamic_includes =
    Memo.List.concat_map dynamic_includes ~f:(fun (loc, include_file) ->
      Memo.push_stack_frame
        ...
        (fun () ->
           let current_file = Include_stanza.file_path include_context loc include_file in
           ...
           let* stanzas, dynamic_includes =
             parse_stanzas
               ast
               ~include_state:(Build { context = include_context; current_file })
               ~eval
           in
           let+ dynamic =
             collect_dynamic_includes eval include_context dynamic_includes
           in
           List.rev_append stanzas dynamic))
  ;;

  let set_dynamic_stanzas t ~context_name ~eval ~dynamic_includes =
    let stanzas =
  ...
  let eval dune_files mask =
    ...
    let+ scripts =
      ...
      |> Memo.parallel_map ~f:(fun (dir, project, dune_file) ->
        ...
        classify ~eval dune_file)
    in
    fun context_name ->
      let* context = Context.DB.get context_name in
      let set_dynamic_stanzas = set_dynamic_stanzas ~context_name in
      Memo.parallel_map scripts ~f:(function
        | Plain { eval; file; static } ->
          let+ dune_file, dynamic_includes = parse static ~file ~eval ~context_name in
          set_dynamic_stanzas dune_file ~eval ~dynamic_includes
        | Script script ->
          let+ dune_file, dynamic_includes =
            Memo.push_stack_frame
              ...
              (fun () -> Script.eval_one ~context_name ~context script)
          in
