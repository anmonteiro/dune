...
let setup_runtime_assets_rules sctx ~dir ~target_dir ~mode
  ...
  in
  let targets = Runtime_deps.targets ~output melange_files in
  let+ () =
    let loc = mel.loc in
...
let setup_entries_js ~sctx ~dir ~dir_contents ~scope ~compile_info ~target_dir
  ...
  let* (_runtime_deps : unit Action_builder.t) =
...
let setup_js_rules_libraries ~dir ~scope ~target_dir ~sctx ~requires_link ~mode
    (mel : Melange_stanzas.Emit.t) =
  let build_js =
    build_js ~sctx ~mode ~module_system:mel.module_system
      ~js_ext:mel.javascript_extension
  in
