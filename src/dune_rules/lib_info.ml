...
let create ~loc ~path_kind ~name ~kind ~status ~src_dir ~orig_src_dir ~obj_dir
    ...
    ~instrumentation_backend ~public_open ~melange_runtime_deps =
  { loc
  ; name
  ; kind
  ; status
...
let map t ~path_kind ~f_path ~f_obj_dir ~f_mrd =
  let f = f_path in
  ...
  ; melange_runtime_deps = Runtime_deps.map ~f:f_mrd t.melange_runtime_deps
  ; path_kind
  }

let map_path t ~f =
  map t ~path_kind:External ~f_path:f ~f_obj_dir:Fun.id ~f_mrd:Fun.id
...
let as_local_exn =
  map ~path_kind:Local ~f_path:Path.as_in_build_dir_exn
    ~f_obj_dir:Obj_dir.as_local_exn ~f_mrd:Path.as_in_build_dir_exn
...
let to_dyn path
  ...
  record
    ...
    ; ("public_open", option Module_name.to_dyn public_open)
