...
module Alias_module = struct
  ...
  module Alias = struct
    ...
    let create ~canonical_path ~local_name ~obj_name =
      { canonical_path = Module_name.Path.to_string canonical_path
      ; local_name
      ; obj_name = Module_name.Unique.to_name ~loc:Loc.none obj_name
      }
    ;;
  ...
  let add_shadowed builder shadowed =
    ...
    String_builder.add_string builder shadowed_definition;
  ...
  let of_modules project modules group instances =
    let aliases =
      ...
      |> List.map ~f:(fun (local_name, m) ->
        Alias.create
          ~canonical_path:(Modules.With_vlib.canonical_path modules group m)
          ~local_name
          ~obj_name:(Module.obj_name m))
    in
    let shadowed =
...
module For_benchmarks = struct
  ...
  let create_input ~aliases ~shadowed = { aliases; shadowed }

  let prepare_and_render { aliases; shadowed } =
    let aliases =
      List.map aliases ~f:(fun (canonical_path, local_name, obj_name) ->
        Alias_module.Alias.create ~canonical_path ~local_name ~obj_name)
    in
    let instances = Parameterised_instances.none in
    let t = { Alias_module.aliases; shadowed; instances } in
    Alias_module.to_ml t
  ;;
end
