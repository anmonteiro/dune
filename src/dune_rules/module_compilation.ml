...
let ocamlc_i_action ~deps ~mode cctx (m : Module.t) =
  let obj_dir = Compilation_context.obj_dir cctx in
  ...
  >>> Command.run'
        ...
        ; parameters cctx
        ; (match mode with
           | Ocamldep.Mode.Standard -> opens (Compilation_context.modules cctx) m
           | Transparent_aliases -> A "-no-alias-deps")
        ; A "-short-paths"
        ; A "-i"
...
let ocamlc_i ~deps ~mode cctx m ~output =
  let sctx = Compilation_context.super_context cctx in
  let dir = Compilation_context.dir cctx in
  ocamlc_i_action ~deps ~mode cctx m
...
let infer_interface cctx m =
  ...
  let action =
    ...
    let+ action =
      ...
      in
      ocamlc_i_action ~deps ~mode:Ocamldep.Mode.Standard cctx m
