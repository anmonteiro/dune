(** A node in the memoization graph: a memoized function applied to an input, together
    ...
    Each node stores its [value], the runs when it last changed and was last
...
module Dep_node : sig
  type ('i, 'o) t = ('i, 'o) M.Dep_node.t =
    ...
    ; mutable last_changed_at : Run.t
    ; mutable last_validated_at : Run.t
      (** [last_changed_at] is the run when the value last changed;
        [last_validated_at] is the run when it was last confirmed up to date.
        Invariant: [last_changed_at <= last_validated_at]. *)
    ; mutable deps : packed Deps.t
