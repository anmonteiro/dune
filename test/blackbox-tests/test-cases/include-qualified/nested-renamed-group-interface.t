...
An executable main can be a renamed group interface. Its implicit interface
uses the physical source filename.

  $ mkdir -p renamed-main/internal
  $ cat >renamed-main/dune-project <<EOF
  > (lang dune 3.25)
  > EOF
  $ cat >renamed-main/dune <<EOF
  > (include_subdirs
  >  (mode qualified)
  >  (dirs (internal as main)))
  > (executable
  >  (name main))
  > EOF
  $ cat >renamed-main/internal/internal.ml <<EOF
  > let () = print_endline "from renamed main"
  > EOF
  $ dune exec --root=renamed-main ./main.exe
  from renamed main
  $ cat renamed-main/_build/default/internal/internal.mli
  (* Auto-generated by Dune *)

The physical name may also come from a copied source, discovered after the
executable's output mask is registered.

  $ mkdir -p copied-main/internal copied-main/inputs
  $ cat >copied-main/dune-project <<EOF
  > (lang dune 3.25)
  > EOF
  $ cat >copied-main/dune <<EOF
  > (include_subdirs
  >  (mode qualified)
  >  (dirs (internal as main)))
  > (executable
  >  (name main))
  > EOF
  $ cat >copied-main/internal/dune <<EOF
  > (copy_files ../inputs/internal.ml)
  > EOF
  $ cat >copied-main/inputs/dune <<EOF
  > (include_subdirs no)
  > EOF
  $ cat >copied-main/inputs/internal.ml <<EOF
  > let () = print_endline "from copied main"
  > EOF
  $ dune exec --root=copied-main ./main.exe
  from copied main
  $ cat copied-main/_build/default/internal/internal.mli
  (* Auto-generated by Dune *)
