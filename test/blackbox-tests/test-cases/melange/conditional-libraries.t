...
other has `(modes :standard)`
  ...
  $ cat prefix/lib/lib_for_melange/dune-package | sed "s/(lang dune .*)/(lang dune <version>)/"
  (lang dune <version>)
  (name lib_for_melange)
  (sections
   (lib
    $TESTCASE_ROOT/prefix/lib/lib_for_melange))
  (files
   (lib
    (META
     dune-package
     melange/foo.ml
     melange/lib_for_melange.cmi
     melange/lib_for_melange.cmj
     melange/lib_for_melange.cmt
     melange/lib_for_melange.ml
     melange/lib_for_melange__Foo.cmi
     melange/lib_for_melange__Foo.cmj
     melange/lib_for_melange__Foo.cmt)))
  (library
   (name lib_for_melange)
   (kind normal)
   (main_module_name Lib_for_melange)
   (modes melange)
   (melange_modules
    (wrapped
     (group
      (alias
       (obj_name lib_for_melange)
       (visibility public)
       (kind alias)
       (source (path Lib_for_melange) (impl (path lib_for_melange.ml-gen))))
      (name Lib_for_melange)
      (modules
       (module
        (obj_name lib_for_melange__Foo)
        (visibility public)
