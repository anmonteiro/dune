...
The design is based on the discussion in
[Dune PR #15493](https://github.com/ocaml/dune/pull/15493) and the earlier
implementation in
...
## Current state and assessment of PR #15493

PR #15493 is already the right mode-agnostic foundation and does not need a
source change for the new UX.
...
- persists `is_default` per configuration;
- changes stanza rule generation to accept a nonempty `Merlin.group`;
- keeps `Processed.get` singular by selecting an applicable default, then
  falling back to the first applicable configuration;
...
Use [Dune PR #15557](https://github.com/ocaml/dune/pull/15557) as the
implementation starting point. It builds on #15493, retains every mode
configuration in one group, and implements the exact-match precedence described
below. Do not revive the older `54d21be196` prototype or its plain-list
representation.
...
- a group is nonempty;
- every member has the same `Merlin_ident`, and therefore one persisted stanza
  file;
- every member has a distinct `Compilation_mode.t`;
- exactly one group member is marked `is_default` (the applicable subset for a
  particular file may omit it);
- ordering is deterministic, with the default first when practical;
...
contexts. Pass the same stanza `Merlin_ident` to every mode and set
`is_default` from the existing `for_merlin` decision. Executable and
`melange.emit` call sites remain singleton groups.
...
incrementing the persistent `merlin-conf` version after #15493's version 9.
Store `Ml_kind.t` with each `per_file_config` entry in the same version bump.
...
PR #15557 implements this classification and precedence for the singular
lookup. Preserve that implementation when extracting the plural
`configurations` API; do not implement the plural lookup as an independent
filter over group members.

Keep `Processed.get` as the compatibility projection of `configurations`:
choose the applicable default, otherwise the first applicable entry.
...
1. **Dune foundation:** merge #15493 unchanged.
2. **Dune generation:** group all library modes, persist mode metadata, and add
   source-kind-aware file matches plus OCaml-only/Melange-only/shared generation
   tests. Keep `File` singular.
3. **Dune protocol:** add the tagged `File-Configurations` request, robust
   stanza search, mode/kind/counterpart encoding, stale-file test, and legacy
   compatibility tests.
